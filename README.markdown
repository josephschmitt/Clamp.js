...
arguments. The first is the DOM object (`$("p")[0]`) which should be clamped, and the second is an
...
Internal clamp utilities are publicly exposed through the global `clampUtils`.

For instance, you can use `clampUtils.getLineHeight()` to get the line height for the DOM object and use that to determine what to do another element.

<pre>
var headline = $("h1")[0];
$clamp(headline, { lines: 2 });

var subheadline = $("h3")[0];
if (clampUtils.getLineHeight(headline) * 2 === headline.clientHeight) {
	subheadNumLines = 1;
} else {
	subheadNumLines = 2;
}
